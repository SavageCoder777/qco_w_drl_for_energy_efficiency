...
def simulate_counts(circuit, repetitions=1024):
    simulator = cirq.Simulator()
    result = simulator.run(circuit, repetitions=repetitions)
    # Convert measurement results into counts
    measured_bits = result.measurements['m']  # assumes measurement key is 'm'
    counts = {}
...
def _adjust_r_based_on_fidelity(circuit, baseline_counts, r_initial=1024, min_r=1, max_r=102400, step=128):
    r = r_initial
    fidelity = 0.0
    while r < max_r:
        agent_counts = cirq_counts(circuit, shots=r)
...
class QuantumPruneEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.r, fidelity = _adjust_r_based_on_fidelity(self.circuit, self.baseline_counts, r_initial=self.r)
