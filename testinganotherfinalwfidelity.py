...
def cirq_counts(circuit: cirq.Circuit, shots=1024):
    ...
    result = simulator.run(circuit, repetitions=int(shots))  # 🔧 ensure integer
...
def get_true_env(env):
    """Unwraps Monitor, DummyVecEnv, VecNormalize, etc., to get the actual custom env."""
...
class SaveMetricsAndRolloutsCallback(BaseCallback):
    def __init__(self, save_freq=256, save_dir="outputs", verbose=1):
        ...
        # Create separate CSV logs
        self.metric_files = {
            'gate_count': open(os.path.join(save_dir, 'gate_count_log.csv'), 'w', newline=''),
            'depth': open(os.path.join(save_dir, 'depth_log.csv'), 'w', newline=''),
            'energy': open(os.path.join(save_dir, 'energy_log.csv'), 'w', newline=''),
            'r_changes': open(os.path.join(save_dir, 'r_changes_log.csv'), 'w', newline=''),
        }

        self.csv_writers = {
            k: csv.writer(f) for k, f in self.metric_files.items()
        }
        for writer in self.csv_writers.values():
            writer.writerow(['step', 'value'])  # CSV header

        self.prev_r = None  # For detecting r changes
    def _on_step(self) -> bool:
        step = self.num_timesteps

        # ✅ Unwrap to access the custom environment
        env = get_true_env(self.training_env.envs[0])
        while hasattr(env, "env"):
            env = env.env

        # Now safe to access custom attributes
        gate_count = env.gate_count
        depth = env.gate_depth
        energy = env.energy
        current_r = env.repetition_count

        # Log values
        self.csv_writers['gate_count'].writerow([step, gate_count])
        self.csv_writers['depth'].writerow([step, depth])
        self.csv_writers['energy'].writerow([step, energy])

        # Only log `r` if it changes
        if self.prev_r != current_r:
            self.csv_writers['r_changes'].writerow([step, current_r])
            self.prev_r = current_r

        # Save rollout data every `save_freq` steps
        if step % self.save_freq == 0:
            ...
            # 📁 Save rollout pickle
            rollout_file = os.path.join(self.rollout_dir, f"rollout_{step:06d}.pkl")
            with open(rollout_file, 'wb') as f:
                pickle.dump(rollout_data, f)

            # 📝 Save human-readable log file alongside .pkl
            log_text = (
                f"[Rollout Saved] Step {step}\n"
                f"Gate Count: {gate_count}, Depth: {depth}, Energy: {energy:.2f}, "
                f"r: {current_r}, Reward: {rollout_data['reward']}\n"
            )
            print(log_text)
            log_txt_path = os.path.join(self.rollout_dir, f"rollout_{step:06d}.txt")
            with open(log_txt_path, "w") as f_txt:
                f_txt.write(log_text)

        # 📣 Print progress every ~100 circuits worth of steps
        if step % (self.save_freq * 100) == 0:
            print(f"[Callback] Processed approximately {step // self.save_freq} circuits...")

        return True


    def _on_training_end(self) -> None:
        for f in self.metric_files.values():
            f.close()
...
def evaluate_on_dataset_with_model(circuits, model):
    ...
    i = 0
    for c in circuits:
        print(f"[EVAL] Optimizing test circuit {i + 1}/{len(circuits)}...")
        ...
        print(f"[EVAL] Circuit {i + 1} done: depth {before['depth']} → {after['depth']}")
        i += 1
    return results
...
if __name__ == '__main__':
    ...
    callback = SaveMetricsAndRolloutsCallback(save_freq=256, save_dir="outputs")
